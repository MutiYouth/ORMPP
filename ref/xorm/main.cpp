...
void demo_multi_thread_use() {
    ...
    // query and update
    {
        ...
        std::cout << "query result: " << r.success << "  " << r.results.size() << std::endl;
        if (!r.results.empty()) {
            auto &info = r.results[0];
            info.date = "2022-10-05";
    ...
    // other type of query
    dao_t<mysql> dao_query_{"xorm"};
    auto r = dao_query_.query<std::tuple<mysql::Integer, std::string, std::string>>("select id, b,d from test");
    auto r0_ = dao_query_.query<test>("");
...
int main() {
    ...
    config.dbname = "test";
    config.host = "192.168.10.106";
    config.password = "root";
    config.user = "weng2022";
    ...
    config2.dbname = "test2_inte";
    config2.host = "192.168.10.106";
    config2.password = "root";
    config2.user = "weng2022";
