...
private:
    ...
    typename std::enable_if<
            is_fundamention_type<typename std::remove_reference<T>::type>::value || is_date_type<typename std::remove_reference<T>::type>::value>::type
...
public:
    template<typename T>
    typename std::enable_if<reflector::is_reflect_class<typename std::remove_reference<T>::type>::value, std::pair<std::int64_t, std::int64_t>>::type
    insert(T &&t) {
    ...
    template<typename T>
    typename std::enable_if<reflector::is_reflect_class<typename std::remove_reference<T>::type>::value, bool>::type
    update(T &&v, std::string const &condition = "") {
    ...
    template<typename T>
    typename std::enable_if<reflector::is_reflect_class<typename std::remove_reference<T>::type>::value, std::pair<bool, std::vector<T>>>::type
    query(std::string const &condition = "") {
        auto meta = meta_info_reflect(T{});
        std::string tablename = meta.get_class_name();
        std::stringstream ss;
        MYSQL_BIND bind[meta.element_size()];
